...
import { DishItem, DayOfWeek, Language } from '../types';
      ...
      <div className="bg-[#FDFCFB] text-[#1A1A1A] rounded-lg max-w-4xl w-full border border-black/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
          ...
          <div className="bg-[#F4F1EA] p-4 rounded-sm border border-black/10">
            <div className="flex items-center gap-2 mb-2 font-serif text-sm font-bold text-[#2D463E]">
              ...
              <span>{t.fixedWeeklyDishesTitle(allWeekDishes.length)}</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {allWeekDishes.map((d) => {
                const locDish = getLocalizedDish(d, language);
                return (
                  <span
                    key={d.id}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {weeklyDays.map((day) => {
              ...
              const dayLabel = getDayLabel(day.id, language);
              ...
              return (
                ...
                >
                  <div>
                    ...
                    {dayDishes.length === 0 ? (
                      <p className="text-xs text-[#1A1A1A]/40 italic py-2">
                        {language === 'en'
                          ? 'Only fixed menu items served on this day.'
                      ...
                      <ul className="space-y-2 mb-3">
                        {dayDishes.map((d) => {
                          ...
                          return (
                            ...
                            >
                              <div>
                                <p className="font-serif font-bold text-[#1A1A1A]">{locDish.name}</p>
                  ...
                  >
                    {language === 'en' ? `View ${dayLabel} Menu` : `Xem Chi Tiết ${dayLabel}`}
                  </button>
                </div>
              );
            })}
        ...
        <div className="bg-[#F4F1EA] px-6 py-4 border-t border-black/10 flex items-center justify-between">
          <p className="text-xs text-[#1A1A1A]/60 font-sans italic">
            {language === 'en'
              ? '* Click "View Menu" button to filter items by that day.'
